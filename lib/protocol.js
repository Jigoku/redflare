...
function Protocol278() { }

//see: src/game/gamemode.h in RE
Protocol278.mutators = {
	ffa           : 1 << 0,
	coop          : 1 << 1,
	instagib      : 1 << 2,
	medieval      : 1 << 3,
...
Protocol278.modeSpecificMutators = {
  gsp1: 1 << 14,
  gsp2: 1 << 15,
  gsp3: 1 << 16
};

Protocol278.modeSpecificMutatorNames = {
  deathmatch: ['gladiator', 'oldschool'],
  'capture-the-flag': ['quick', 'defend', 'protect'],
  'defend-and-control': ['quick', 'king'],
  'bomber-ball': ['hold', 'basket', 'assault'],
...
//see: src/game/gamemode.h in RE
Protocol278.gameModes = [
	'demo',               // 0
	'edit',               // 1
	'deathmatch',         // 2
	'capture-the-flag',   // 3
	'defend-and-control', // 4
	'bomber-ball',        // 5
	'speedrun',           // 6
];

Protocol278.prototype.mutatorsFromFlags = function(flags, gameMode) {
	var muts = [];
	_.each(_.keys(Protocol278.mutators), function(mutator) {
	  if (flags & Protocol278.mutators[mutator]) { muts.push(mutator); }
	});
  var idx = 0;
  if (gameMode) {
    var gameModeName = (gameMode < Protocol278.gameModes.length) && Protocol278.gameModes[gameMode] || null;
    var gameSpecificMutators = Protocol278.modeSpecificMutatorNames[gameModeName];
    if (gameSpecificMutators) {
    	_.each(_.keys(Protocol278.modeSpecificMutators), function(mutator) {
    	  if (flags & Protocol278.modeSpecificMutators[mutator]) {
          if (idx < gameSpecificMutators.length) {
            muts.push(gameSpecificMutators[idx]);
          }
        }
...
Protocol278.prototype.masterModeFromCode = function(code) {
	if (code < 0 || code > Protocol278.masterModes.length) {
	  return 'unknown';
	} else {
	  return Protocol278.masterModes[code];
	}
};

Protocol278.prototype.gameModeFromCode = function(code) {
	if (code < 0 || code > Protocol278.gameModes.length) {
	  return 'unknown';
	} else {
	  return Protocol278.gameModes[code];
	}
};
...
if (typeof exports == 'object') {
  ...
  exports.Protocol278 = Protocol278;
}
